...
- Nota técnica: la primera corrida de prueba (misma fecha del lote manual, 20 de agosto) se saltó sola por la regla de no-duplicar del prompt de la rutina — comportamiento esperado, igual que pasó en Danny Mera Seguros el 12 de agosto. Se intentó forzar una segunda corrida de prueba ese mismo día agregando temporalmente una nota al prompt pidiéndole ignorar el chequeo anti-duplicado; la rutina **se negó** a hacerlo, tratando esa nota como una instrucción sospechosa insertada en su propio prompt (pidió confirmación explícita en vivo antes de saltarse su salvaguarda) y avisó por notificación push sin hacer ningún push a git — buena señal de que la automatización no sigue instrucciones raras a ciegas. El prompt se volvió a dejar en su versión limpia. La validación real de que el push funciona queda pendiente para la primera corrida real, el 21 de agosto.

## Sincronización local de carruseles (`Contenido Carrusel/Carrusel`)
- Script: `scripts/sync_carruseles.py` — copia lo nuevo de `carruseles/<YYYY-MM-DD>/carrusel-N-tema/` (que llega vía `git pull` desde la rutina en la nube) hacia `Contenido Carrusel/Carrusel/<Mes Año>/<día>/C<n>/`, deduplicando por hash de la primera imagen (así no pisa nada que Nicole haya guardado a mano en esa misma carpeta del día). Copiado tal cual del mismo script de Danny Mera Seguros, solo cambia la ruta del repo (`REPO = /Users/danny/Contabilidad NC`).
- **Pendiente:** falta que Danny arme el `launchd` local en su Mac para que corra solo todos los días (plist en `~/Library/LaunchAgents/`, mismo patrón que Danny Mera Seguros: `com.dannymera.seguros.synccarruseles.plist`, corriendo ~2 horas después de la rutina en la nube, es decir cerca de las 10:00 AM hora del Mac). Mientras tanto se puede correr a mano con `python3 scripts/sync_carruseles.py` desde `/Users/danny/Contabilidad NC`.
- `Contenido Carrusel/` es local, **no se sube al repo** (está en `.gitignore`) — es solo para que Nicole la revise y publique manualmente en redes, igual que la página `carruseles.html` no listada, pero organizada en carpetas de Finder en vez de una web.
- Logs en `scripts/sync_carruseles.log` (log propio del script) y, una vez armado el launchd, `scripts/launchd_out.log` / `scripts/launchd_err.log` (salida de launchd) — revisar ahí primero si algún día no aparece contenido nuevo.
